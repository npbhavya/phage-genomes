...
if config ['sequencing'] == 'paired':
    ...
    SAMPLES2,EXTENSIONS2, =glob_wildcards(os.path.join(READDIR, '{sample}.{extn}'))
    if len(SAMPLES2) ==0:
        sys.stderr.write(f"We did not find any fastq files in {SAMPLES2}. Is this the right read dir?\n")
        sys.stderr.write(f"If the files are there, but running into an error, check filepaths\n")
        sys.exit(0)
    if len(set(EXTENSIONS2)) != 1:
        sys.stderr.write("FATAL: You have more than one type of file extension\n\t")
        sys.stderr.write("\n\t".join(set(EXTENSIONS2)))
        sys.stderr.write("\nWe don't know how to handle these\n")
        sys.exit(0)

    FQEXTN2 = EXTENSIONS2[0]
    PATTERN = '{sample}.'+FQEXTN2
...
if (os.path.exists(OUTDIR)==True):
    PHAGE, EXTN = glob_wildcards(os.path.join(OUTDIR, "phage_finalset", "{sample}.{ext}"))
    if (len(PHAGE) == 0):
    ...
    if len(set(EXTN)) != 1:
        ...
        QCDIR = os.path.join(OUTDIR, 'prinseq')
        FILDIR = os.path.join(OUTDIR, 'filtlong')
        GENOMEDIR = os.path.join(OUTDIR, 'phage_finalset')
       
        PHAGEIL=list(set(SAMPLES).intersection(PHAGE))
        PHAGENANO=list(set(SAMPLES2).intersection(PHAGE))
        #print(f"Illumina subset {PHAGEIL}")
"""
